...
void ExperienceManager::level_up_character(Entity character_entity) {
	if (!registry.levelUpTimers.has(character_entity)) {
		...
		Entity level_text = createText("LEVEL UP!", vec2(caller_motion.position.x + 50.f, window_height_px - caller_motion.position.y), 0.7f, vec3(0.0f, 0.0f, 1.0f), mat4(1.0f), StageSystem::Stage::turn_based, false);
